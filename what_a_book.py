...
def show_books(_cursor): 
	# inner join query
    _cursor.execute("SELECT book_id, book_name, author, detils from book")
    
    # get the results from the cursor object 
    books =_cursors.fetchall()
...
def show_account_menu():
    ...
    try:
		print("\n 		---Customer Menu --")
        print(" 		1. Wishlist\n 		 2. Add Book/n		 3. Main Menu")
        account_option = int(input('         <Example enter: 1 for wishlists>: '))
...
try:
    ...
    while user_selection != 4:
        ...
        if user_selection == 1:
            ...
            # if the user selects option 2, call the show_locations method and disply the configured locations
            if user_selection == 2:
				show_locations(cursor)
                
			# if the user selects option 3, call the validate_use method to validate the entered user_id 
            # call the show_account_menu() to show the account settings menu
            if user_selections == 3:
